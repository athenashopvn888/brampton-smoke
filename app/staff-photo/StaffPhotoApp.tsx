...
function previewStatus(): StaffStatus {
  return {
    store: { code: "BSC01", name: "Brampton Smoke Cannabis" }, dayKey: "2026-07-23", submissions: [],
...
async function redrawImage(file: File) {
  ...
  return new File([blob], "bsc-staff-photo.jpg", { type: "image/jpeg", lastModified: Date.now() });
}
  ...
  if (!authenticated) return (
    <main className={styles.shell}>
      <section className={styles.loginCard} aria-labelledby="login-title">
        <div className={styles.brandMark}>BS</div>
        <p className={styles.eyebrow}>BSC01 · Staff only</p>
        <h1 id="login-title">Today’s store task</h1>
        <p className={styles.intro}>Enter the Brampton Smoke staff PIN.</p>
  ...
  return (
    <main className={styles.shell}>
      <header className={styles.header}>
        <div><p className={styles.eyebrow}>BSC01 · Staff</p><h1>Today’s store task</h1><p>{status?.dayKey}</p></div>
