...
import BSCWebChat from "./BSCWebChat";
...
export default function DeliveryContent() {
  ...
  async function handleEmailSubmit(event: React.FormEvent) {
    ...
    try {
      await fetch(`https://script.google.com/macros/s/AKfycbySrZYxI-NNnXfxY1jXOqHgT2HQi4zst2Fgte6FXTeymat_W_r0o1E3P83EfnVCjEk0/exec?action=delivery_email&email=${encodeURIComponent(email)}&store=BSC01`, { method: "GET", mode: "no-cors" });
    } finally {
  ...
  useEffect(() => {
    ...
    fetch("https://milestone-1-demo.vercel.app/api/catalog?store=BSC", { signal: controller.signal })
  ...
  return <main className={styles.main}>
    ...
    <section className={styles.howToOrder} aria-labelledby="how-to-order-title">
      ...
      <ol>
        ...
        <li><strong>Confirm with the dispatcher</strong><span>The Brampton Smoke Cannabis dispatcher confirms availability, delivery details, and next steps.</span></li>
      </ol>
    </section>
      ...
      <div className={styles.catalog}>
        ...
        <div className={styles.grid}>{visible.map((product) => <article className={styles.card} key={product.publicProductId}>
          <button type="button" className={styles.imageButton} onClick={() => setSelected(product)} aria-label={`View details for ${product.name}`}>
            {product.images[0] ? <Image src={product.images[0]} alt={`${product.name} on the Brampton Smoke Cannabis delivery menu`} fill sizes="(max-width: 640px) 50vw, 280px" unoptimized /> : <span>Brampton Smoke Cannabis</span>}
          </button>
    ...
    <section className={styles.updates} aria-labelledby="delivery-updates-title">
      <div><p>DELIVERY UPDATES</p><h2 id="delivery-updates-title">Get store delivery updates</h2><span>Enter your email to keep the existing Brampton Smoke Cannabis delivery notification active.</span></div>
