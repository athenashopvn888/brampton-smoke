...
export default function BSCWebChat() {
  ...
  useEffect(() => {
    const initial = window.setTimeout(() => {
      if (token) void refresh(token).catch(() => setNotice("Your previous Brampton Smoke Cannabis chat could not be restored."));
  ...
  async function start(event: FormEvent) {
    ...
    try {
      const data = await payload(await fetch(`${API_BASE}/api/web-chat/session`, {
        ...
        body: JSON.stringify({ storeId: "BSC", customerName: intent === "NEW_CUSTOMER" ? name : "", phone, intent, message: firstMessage, smsConsent }),
      }));
  ...
  return (
    <aside className={`sod-web-chat ${open ? "open" : ""}`} aria-label="Brampton Smoke Cannabis Web Chat">
      ...
      {open && <section className="sod-chat-panel" role="dialog" aria-modal="true" aria-label="Brampton Smoke Cannabis Web Chat">
      <header><div><strong>Brampton Smoke Cannabis Web Chat</strong><small>Start your delivery order with a dispatcher</small></div><button type="button" onClick={() => setOpen(false)} aria-label="Minimize chat">×</button></header>
